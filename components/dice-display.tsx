...
// Component to display a dice with a falling, bouncing, and spinning animation that reveals its number.
function DiceRoll({ value, sides = 20 }: { value: number; sides?: number }) {
  // Generate random values for a variable spin speed and throw angle
  const randomX = useMemo(() => (Math.random() - 0.5) * 100, []); // random horizontal offset between -50 and 50
  ...
  const randomRotateX = useMemo(() => Math.random() * 90 - 45, []); // rotation angle on X-axis between -45 and 45 degrees
  const randomRotateY = useMemo(() => Math.random() * 90 - 45, []); // rotation angle on Y-axis between -45 and 45 degrees
  ...
  return (
    <motion.div
      ...
      transition={{
        ...
        y: { type: "spring", stiffness: 300, damping: 10, bounce: 0 },
        rotate: { duration: randomSpinDuration, ease: "linear" },
        rotateX: { duration: randomSpinDuration, ease: "linear" },
        rotateY: { duration: randomSpinDuration, ease: "linear" },
      ...
      <div className="p-6 rounded-lg border bg-card text-card-foreground shadow-sm">
        <div className="dice-rolls flex gap-4 justify-center">
          {diceRolls.map((dice, index) => (
            <DiceRoll
              key={index}
              value={(dice as any).result as number}
